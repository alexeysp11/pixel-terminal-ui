...
Использование плоского `switch-case` на базе `Command<TEnum>` обеспечивает максимально быстрое O(1) переключение состояний в рантайме. Однако для комплексных бизнес-процессов (где цепочка шагов складской приемки может содержать десятки ветвлений) ручное описание таких переходов снижает читаемость кода.
* **Решение:** В промышленной эксплуатации оптимальным решением этой проблемы станет использование **C# Source Generators**. Разработчик сможет описывать многошаговые сценарии в элегантном декларативном Fluent API, а кодогенератор на этапе компиляции развернет его в аналогичный высокоэффективный и плоский `switch-case`, сохранив и чистоту архитектуры, и Zero Allocation в рантайме.
