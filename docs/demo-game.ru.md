...
* **Инлайновый курсор ввода:** Сервер передаёт клиенту координаты активного поля вместе с кадром, и клиент позиционирует курсор терминала прямо внутри отрисованного виджета формы — печатать команды нужно прямо там, где мигает курсор, отдельной строки ввода под формой больше нет.
